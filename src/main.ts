/**
 * 文章批注插件。
 * 逻辑写在 src/，根目录 main.js 由 esbuild 从这里生成。
 */
import {
  App,
  Editor,
...
// ==================== i18n System ====================
type LocaleNode = string | { [key: string]: LocaleNode | undefined };
...
function t(key: string, plugin?: { settings?: { language?: string } }): string {
...
function getColorName(color: string, plugin?: { settings?: { language?: string } }): string {
  const lang = plugin?.settings?.language || "zh";
...
function getFileType(file: { extension?: string } | null | undefined): "pdf" | "markdown" {
  return file?.extension === "pdf" ? "pdf" : "markdown";
}
function isMarkdownPosition(position: AnnotationPosition | LoosePosition | null | undefined): position is MarkdownPosition {
  return !!position && (position.kind === "markdown" || ("startLine" in position && typeof position.startLine === "number"));
}
function isPdfPosition(position: AnnotationPosition | LoosePosition | null | undefined): position is PdfPosition {
  return !!position && position.kind === "pdf";
}
function normalizeRect(rect: Partial<PdfRect> | null | undefined): PdfRect | null {
  if (!rect)
    return null;
...
function normalizeAnnotation(annotation: AnnotationDraft | null | undefined): Annotation | null {
...
function getAnnotationLocationLabel(annotation: Annotation, plugin?: { settings?: { language?: string } }): string {
...
function getCodeMirror(editor: Editor): EditorView | null {
  const cm = (editor as Editor & { cm?: EditorView }).cm;
  return cm ?? null;
}

function refreshHighlights(plugin: ArticleAnnotator): void {
...
function formatTime(ts: number, plugin?: { settings?: { language?: string } }): string {
  const d = new Date(ts);
...
// ==================== 颜色验证 ====================
function validateHexColor(hex: unknown): hex is string {
...
export default class ArticleAnnotator extends Plugin {
  ...
  getActivePdfView(): FileView | null {
    ...
    if (activeView instanceof FileView && activeView.file?.extension === "pdf")
      return activeView;
    const matching = leaves.find((leaf) => leaf.view instanceof FileView && leaf.view.file?.path === this.activeFile?.path);
  ...
  capturePdfSelection() {
    ...
    if (!startPage || !endPage)
      return null;
    if (startPage !== endPage) {
      new Notice(t("notifications.crossPageNotSupported", this));
      return null;
    }
  ...
  async highlightPdfSelection(selection: PdfSelection | null, color: string) {
    if (!selection)
      return;
    const existing = this.getAnnotationsForFile(selection.filePath).filter((ann): ann is Annotation & { position: PdfPosition } => ann.fileType === "pdf" && isPdfPosition(ann.position) && ann.position.page === selection.page);
    ...
    new Notice(t("notifications.highlightAdded", this).replace("${color}", getColorName(color, this) || color));
  }
  ...
  renderPdfHighlights(filePath = this.activeFile?.path) {
    ...
    if (!pageElements.length)
      return;
    const annotations = this.getAnnotationsForFile(filePath).filter((ann): ann is Annotation & { position: PdfPosition } => ann.fileType === "pdf" && isPdfPosition(ann.position));
  ...
  async readAnnotationStoreFile(filePath: string): Promise<AnnotationStoreData | null> {
    ...
    try {
      ...
      new Notice(t("notifications.syncedStoreReadFailed", this));
      return null;
    }
  }
  ...
  async migrateLegacyPluginData(settingsFallback: AnnotatorSettings | null = null) {
    ...
    if (migratedAnnotations.length > 0 || migratedGroups.length > 0) {
      ...
      new Notice(t("notifications.annotationsMigrated", this));
    }
  ...
  async clearFileAnnotations() {
    ...
    new Notice(t("notifications.fileCleared", this).replace("${n}", String(count)));
  }
  // ==================== 分组管理 ====================
...
class SearchModal extends Modal {
  ...
  onOpen() {
    ...
    const doSearch = (query: string) => this.renderResults(query);
